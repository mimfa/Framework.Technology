...
namespace MiMFa.Exclusive.Technology.MARC
{
    public class MakerMnemonics
    {
        public static string ReplaceCharsWithMnemonics(string aString)
        {
            ...
            {
                int charValue = System.Convert.ToInt32(aString[i]);
                if (charValue >= 0x7F || charsToChangeBelow7F.Contains(charValue))
                {
                    ...
                    try
                    {
                        cleanString.Append(MakerFeatures.HexToMnemDictionary[aString[i]]);
                    }
                    catch
                    { 
        ...
        public static string ReplaceMnemonicsWithChars(string aString)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                if(MakerFeatures.MnemToHexDictionary.Keys.Contains(match.ToString()))
                {
                    cleanString = cleanString.Replace(match.ToString(), MakerFeatures.MnemToHexDictionary[match.ToString()].ToString());
                }
            }
            return cleanString;
