...
namespace MiMFa.Exclusive.Technology.MARC
{
        ...
        //2. Check for cases with 'char' makes more sense than 'string'
    {
        ...
        public Field(int tag, string[] indicators, List<string> subfields, string data)
        {
        ...
        public Field(int tag, string[] indicators, List<string> subfields)
        {
            this.tag = tag.ToString("000");
            for (int i = 0; i < indicators.Length; i++)
            {
        ...
        public Field(int tag, string data)
        {
            this.tag = tag.ToString("000");
            this.Data = data;
        ...
        public string MARCMakerFormat()
        {
            ...
            {
                returnText.Append(
                    MakerMnemonics.ReplaceCharsWithMnemonics(
        ...
        public string AsMARC21()
        {
            if (this.IsControlField())
            {
                return this.Data + Features.END_OF_FIELD;
            }
            string marc = this.Indicator1 + this.Indicator2;
            for (int i = 0; i < this.subfields.ToArray().Length; i = i + 2)
            {
                marc += Features.SUBFIELD_INDICATOR+this.subfields[i] + this.subfields[i + 1];
            }
            return marc + Features.END_OF_FIELD;
        }
